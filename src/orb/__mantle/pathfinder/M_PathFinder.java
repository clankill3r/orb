...
import static orb.____inner_core.IC_Common.swapWithLastRemove;
...
public class M_PathFinder<T> {
    ...
    _GetFloat_TT<T> getAdjacentCost;
    _GetFloat_TT<T> getEstimatedCost;

    ArrayList<M_FGHNode<T>> open = new ArrayList<>();


    public M_PathFinder(_FGHNodeProvider<T> fghNodeProvider, _GetIterator_T<T> getLinkIterator, _GetFloat_TT<T> getAdjacentCost, _GetFloat_TT<T> getEstimatedCost) {
            ...
            while (itr.hasNext()) {
                ...
                if (!neighbourNode.closed) {
                    ...
                    if (!neighbourNode.isInOpenList) {
                        ...
                        float cost = getAdjacentCost.val(current, neighbour);
